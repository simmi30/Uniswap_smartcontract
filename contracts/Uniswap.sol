...
contract Uniswap {
    ...
    constructor(address _token){
        require(_token != address(0), "It means wrong");
        tokenAddress = _token;
        ShikhaToken = IERC20(_token); 
    }
    ...
    function addLiquidity(uint256 _tokenAmount) external payable {
        ...
        ShikhaToken.transferFrom(msg.sender, address(this), _tokenAmount);
        ShikhaToken.approve(ROUTER, _tokenAmount);
    ...
    function withdrawETH(uint256 _amount) external {
        require(ETHStored[msg.sender] >= _amount, 'Nothing');
